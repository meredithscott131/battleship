...
/**
 * Representation of a player in BattleSalvo controlled by the user of the console.
 */
public class UserPlayer implements Player {
  ...
  /**
   * Instantiates a new UserPlayer.
   *
   * @param data the data of the player.
   * @param seed the randomness of the ship placement.
   */
  public UserPlayer(UserData data, int seed) {
    this.data = data;
   ...
   * @param data the data of the player.
   */
  public UserPlayer(UserData data) {
    this.data = data;
    this.rand = new Random();
  }

  /**
   * Instantiates a new UserPlayer given a Random.
   *
   * @param data the data
   * @param rand the randomness of the placement of the ships on the player's game board
   */
  public UserPlayer(UserData data, Random rand) {
    this.data = data;
    this.rand = rand;
  }
  ...
  public List<Coord> reportDamage(List<Coord> opponentShotsOnBoard) {
    ...
    for (Coord coord : opponentShotsOnBoard) {
      int y = coord.getY();
      int x = coord.getX();
      for (int i = 0; i < data.getBoard().getGameBoard().length; i++) {
        for (int j = 0; x < data.getBoard().getGameBoard()[i].length; x++) {
          if (y == data.getBoard().getGameBoard()[i][j].getY()
              && x == data.getBoard().getGameBoard()[i][j].getX()) {
            hits.add(data.getBoard().getGameBoard()[i][j]);
          }
        }
      }
    }
